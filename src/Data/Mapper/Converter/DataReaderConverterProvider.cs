...
{
    public class DataReaderConverterProvider : IConverterProvider
    {
        ...
        {
            return new DataReaderConverter<TDestination>(mapper, source);
        }

        private class DataReaderConverter<TDestination> : IDataConverter<TDestination>
        {
            private readonly IDataReader _reader;
            private readonly IMapper<TDestination> _mapper;
            private readonly IPropertyValue<TDestination>[] _properties;
            public DataReaderConverter(IMapper<TDestination> mapper, object datasource)
            {
                _reader = (IDataReader)datasource;
                _mapper = mapper;
            ...
            public List<TDestination> Cast()
            {
                ...
                while (_reader.Read())
                {
                    list.Add(Create(_reader));
                }
                return list;
            }

            private TDestination Create(IDataRecord dataRecod)
            {
                ...
                for (int i = 0; i < _properties.Length; i++)
                {
                    if (_properties[i] != null)
                        _properties[i].SetValue(output, dataRecod[i]);
                }
            ...
            {
                var array = new IPropertyValue<TDestination>[_reader.FieldCount];
                var fields = _mapper.MetaInfo.GetFieldNames();
                // 考虑数据库里面不区分大小写，但.net里面区分。
                for (int i = 0; i < array.Length; i++)
                {
                    string field = _reader.GetName(i);
